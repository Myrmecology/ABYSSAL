...
(function () {
  ...
  function startDrone() {
    ...
    frequencies.forEach((freq, i) => {
      ...
      // Slowly detune each oscillator for unsettling beating
  ...
  function startStaticCrackle() {
    ...
    for (let i = 0; i < bufferSize; i++) {
      // Sparse crackle — mostly silence, occasional pop
  ...
  // --- High whisper tone (unsettling high freq presence) ---
  function startWhisperTone() {
    const osc    = ctx.createOscillator();
    const gain   = ctx.createGain();
    const filter = ctx.createBiquadFilter();
  ...
  // --- One-shot: glitch burst (call on interactions) ---
  function playGlitch() {
    if (!ctx) return;
    const now = ctx.currentTime;
  ...
  // --- One-shot: cart add confirmation (unsettling) ---
